...
#include <copernicus_base/controlsConfig.h>
...
uint8_t incomplete[2048];
uint16_t incompleteSize=0;
bool foundIncomplete=false;

uint16_t checkSum(char *buff);
void findData(uint8_t *data , uint8_t size);
uint16_t findsequence(uint8_t* data,uint8_t size);
	...
	* */
	...
	while(dataOccurance !=0)
	{
		...
		if(dataOccurance >0)
		{
			...
			copernicus_data_callBack(tempData+dataOccurance);
